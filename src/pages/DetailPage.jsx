...
function DetailPage() {
  ...
  useEffect(() => {
    ...
    dispatch(asyncReceiveTalkDetail(id)); // @TODO: dispatch async action to get talk detail by id
  }, [id, dispatch]);

  const onLikeTalk = () => {
    // @TODO: dispatch async action to toggle like talk detail\
    dispatch(asyncToogleLikeTalkDetail(talkDetail.id)); // @TODO: dispatch async action to toggle like talk detail
  };

  const onReplyTalk = (text) => {
    // @TODO: dispatch async action to add reply talk
    dispatch(asyncAddTalk({ text, replyTo: talkDetail.id })); // @TODO: dispatch async action to add reply talk
  };
