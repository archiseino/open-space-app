...
function HomePage() {
  ...
  useEffect(() => {
    ...
    dispatch(asyncPopulateUsersAndTalks()); // @TODO: dispatch async action to populate talks and users data
  }, [dispatch]);

  const onAddTalk = (text) => {
    // @TODO: dispatch async action to add talk
    dispatch(asyncAddTalk({ text })); // @TODO: dispatch async action to add talk
  };

  const onLike = (id) => {
    // @TODO: dispatch async action to toggle like talk
    dispatch(asyncToogleLikeTalk(id)); // @TODO: dispatch async action to toggle like talk
  };
